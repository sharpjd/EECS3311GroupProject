...
public class DBNew {
	...
	public void addRandomRatingsToMovies() {
	    Random random = new Random();
	    String query = "MATCH (m:Movie) RETURN m.movieid AS movieId";

	    try (Session session = DBUtil.getSession()) {
	        List<Record> movies = session.run(query).list();
	        for (Record movie : movies) {
	            String movieId = movie.get("movieId").asString();
	            double randomRating = Math.round(random.nextDouble() * 100.0) / 10.0;

	            String updateQuery = "MATCH (m:Movie {movieid: $movieId}) " +
	                                 "SET m.rating = $rating";

	            session.run(updateQuery, parameters("movieId", movieId, "rating", randomRating));
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
